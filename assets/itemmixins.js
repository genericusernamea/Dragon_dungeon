...
/*
    Basic Template
*/
Game.ItemMixins.Basic = {
    name: 'Basic',
    init: function(template){
        this._isGeneric = template['isGeneric'] || false;
    }
}
Game.ItemMixins.Value = {
    name: 'Value',
    init: function(template){
        this._value = template['value'] || 0;
        if(this._value !== 0){
            this._isSellable = true;
        }else{
            this._isSellable = false;
        }
        this._buyPrice = template['buyPrice'] || 0;
        if(this._buyPrice !== 0){
            this._isBuyable = true;
        }else{
            this._isBuyable = false;
        }
        this._sellPrice = template['sellPrice'] || 0;
        if(this._sellPrice !== 0){
            this._isSellable = true;
        }else{
            this._isSellable = false;
        }
        this._setCatagory = template['setCatagory'] || 'generic';
    }
}
// Edible mixins
